...
const EditMedia = props => {
  ...
  //   const [Medias, getContents] = useState([]);
  ...
  const getSingleMedia = () => {
    axios
      .get(CONSTANTS.BASE_URL + 'api/media/file/' + id)
      .then(response => {
        console.log('get resoposldsl', response.data);
        const mediaData = response.data;
        
        setMediaData(mediaData);
        setProfileImageFile(CONSTANTS.BASE_URL + mediaData.image_url);
        // setIconImage(CONSTANTS.BASE_URL + mediaData.icon_url);
        // setImage(CONSTANTS.BASE_URL+mediaData.imageUrl);
        //   getBills(allBills);
      })
      .catch(error => {
        console.log(error);
  ...
  // const getAllMedias = () => {
  //   axios.get(CONSTANTS.BASE_URL+'api/media',{params:{limit:1000,page:1}})
  //   .then(response => {
  //   //   console.log("service data",response.data);
  //     const allMedias = response.data.medias;
  //     getMedias(allContents);
  ...
  //     if (isEmpty(medias)) setDisabledSubmit(true);
  //     return true;
  //   } else {
  ...
  // const getRolesWithContentId = mediaId => {
  //   // setRoles([]);
  //   axios
  //     .get(CONSTANTS.BASE_URL + 'api/role', {
  //       params: { page: 1, limit: 10000, media_id: mediaId }
  //     })
  //     .then(response => {
  //       // console.log("+++++++++++++++++++",response.data.roles);
  //       const allRoles = response.data.roles;
  ...
  let page =
    mediaData && associates ? (
      <Page className={classes.root} title="Update Media">
      ...
      > */}
        <Container maxWidth="md">
          <Formik
            initialValues={{
              media_type: mediaData.type === 'url' ? 'url' : 'file_upload',
              name: mediaData.name,
              duration: mediaData.duration,
              type: mediaData.type,
              resolution: mediaData.type === 'url' ? '' : mediaData.resolution,
              active: mediaData.active,
              transition: mediaData.transition,
              active_date: mediaData.active_date ? mediaData.active_date : null,
              expire_date: mediaData.expire_date ? mediaData.expire_date : null,
              // url: mediaData.ty,
              height:
                mediaData.type === 'url'
                  ? mediaData.resolution.split('x')[1]
                  : '',
              width:
                mediaData.type === 'url'
                  ? mediaData.resolution.split('x')[0]
                  : '',
              media_url: mediaData.type === 'url' ? mediaData.url : ''
            }}
            ...
            onSubmit={values => {
              ...
              if (values.media_type !== 'url' && !isUndefined(values.url)) {
              ...
              axios
                .put(
                  CONSTANTS.BASE_URL + 'api/media/file/' + mediaData._id,
                  data
                )
                .then(response => {
